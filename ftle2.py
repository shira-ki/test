...
def read_data(t):
    filename = f"test/vel-{t}.txt"
    with open(filename) as f:
        lines = f.readlines()
        #match = re.match(r'\[(\d+),(\d+),(\d+)\] = \[(.+),(.+),(.+)\]', lines[0])
        #nx, ny, nz = int(match.group(1)), int(match.group(2)), int(match.group(3))
        #nx, ny, nz = [int(s) for s in re.findall(r'\d+', lines[0])]
        uu = np.zeros((nx, ny, nz))
        vv = np.zeros((nx, ny, nz))
        ww = np.zeros((nx, ny, nz))
        for line in lines[1:]:
            i, j, k, u, v, w = [float(s) for s in re.findall(r'[-+]?\d*\.\d+|[-+]?\d+', line)]
            uu[int(i), int(j), int(k)] = u
            vv[int(i), int(j), int(k)] = v
            ww[int(i), int(j), int(k)] = w
    return uu, vv, ww
...
inttime = 2.4
delt = 0.1    # time difference between adjacent data files
delta = 0.1    # interation time step
direction = -1
method = 1
...
for t in range(0, int(inttime/delta)):
    print ('time: ' + str(t_start) + ' (' + dir_str + '), integration time: ' + str(t+1) + '/' + str(int(inttime/delta)))
